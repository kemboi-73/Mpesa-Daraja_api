...
def process_stk_callback(request):
    ...
    try:
        ...
        if result_code == 0 and "CallbackMetadata" in stk_callback:
            ...
            # Parse TransactionDate to Python datetime
            raw_transaction_date = get_metadata_value("TransactionDate")

        if raw_transaction_date:
            try:
                parsed_transaction_date = datetime.strptime(str(raw_transaction_date), "%Y%m%d%H%M%S")
            except ValueError:
